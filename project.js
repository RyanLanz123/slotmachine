...
const spin = () => {
    ...
    const reels = []; 
    for (let i = 0; i < COLS; i++){ //loops reels
        reels.push([]);
...
const transpose = (reels) => {
    ...
    for(let i=0; i < ROWS; i++){
        ...
        for (let j = 0; j < COLS; j++){
            rows[i].push(reels[j][i])//accesses columns then returns rows
        }
    }
...
const printRows = (rows) => {
    for (const row of rows) {
        ...
        for(const [i, symbol] of row.entries()){
            rowString += symbol //concactinate elements into the string
            if (i != row.length -1){
                rowString += " | "
            }
        }
...
const getWinnings = (rows, bet, lines) => {
    ...
    for (let row = 0; row < lines; row++){
        ...
        let allSame = true; // means won

        for (const symbol of symbols){ //loops through all symbols
            if (symbol != symbols[0]){ 
                allSame = false; //means didn't win
...
const game = () => {
    ...
    if (balance <= 0) {
        console.log("You have no credit left!")
        break;
    }
